...
1. **新增 |t - TOE| vs 误差散点图**
   ...
   脚本现在会生成第二个图窗口，展示误差与星历外推时间的关系。横轴为`|t - TOE|`（观测时刻与星历参考时刻的时间差），纵轴为3D位置误差。这个图可以直观展示"离TOE越远误差越大"的规律。

2. **按t-toe分组计算平均误差**

   由于一天内同一颗卫星会使用多套星历，相同的`|t - TOE|`值会出现多次。脚本会把这些相同t-toe的误差值取平均，用红色折线连接，展示误差随外推时间增长的平均趋势。

3. **t-toe值对齐到15分钟**

   由于SP3精密星历每15分钟才有一个数据点，而coordinates.txt中存储的`|t - TOE|`是根据实际TOE计算的（可能不是15分钟的整数倍），会导致散点图上出现散乱的小数点。现在将t-toe四舍五入到最近的900秒（15分钟）倍数，使散点图更整齐。

4. **TOE点标记改进**

   - 原来从RINEX文件读取TOE列表来判断哪些是TOE时刻，但这与coordinates.txt使用的星历不一定匹配
   - 现在直接使用coordinates.txt第6列输出的`|t - TOE|`值：小于60秒的点用实心标记，其余用空心
   - 这样确保了时间序列图和t-toe散点图的数据来源一致
...
将精密星历（SP3）视作真值，对比广播星历计算结果：

1. **误差与外推时间无明显相关性**

   从 |t-TOE| vs 误差散点图来看，3D位置误差的大小与星历外推时间 |t-TOE| 之间未见明显相关性——各时段的误差分布范围基本一致。

2. **误差没有系统性增长规律**

   从时间序列来看，XYZ各分量误差随时间波动，但不呈现系统性的增长规律，而是围绕某个均值随机波动。

3. **结论**

   广播星历与精密星历的偏差主要源于**轨道参数的拟合精度**，而非外推过程中的误差累积。在星历有效期内（±2小时），外推时间对定位精度的影响不显著。典型误差范围为 **0.5-4米**。

---

### 发现5：C19异常星历

C19在17:00存在两条星历，其中一条有问题：

| 属性 | 异常星历 | 正常星历 |
|------|---------|---------|
| toc | 17:00 | 17:00 |
| TOE | 56808秒 (15:46) | 61200秒 (17:00) |
| toc-toe差 | 4392秒 | 0秒 |
| 轨道倾角 i0 | **-0.86 rad (-49°)** | +0.96 rad (+55°) |

异常星历的 **轨道倾角为负值**（正常应为+55°），会导致在 t≈56808秒附近计算出错误的卫星位置，产生轨迹跳变。
...
更严重的是，21:00星历的 **Omega0 = +1.27 rad**，而其他星历约为 **-2.89 rad**，相差约 **238°**，会导致轨迹严重不连续。
